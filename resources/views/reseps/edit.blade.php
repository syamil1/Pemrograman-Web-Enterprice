...
<div class="container py-5">
    <a href="/account" class="btn-back-dashboard">Kembali</a>
    <h2 class="text-center mb-4" style="color: #DBAF50;">Edit Resep Anda</h2>
    <form action="{{ route('resep.update', $resep->id) }}" method="POST" enctype="multipart/form-data" class="upload-form p-4 shadow-lg rounded">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="title" class="form-label">Judul Resep</label>
            <input type="text" id="title" name="title" class="form-control" placeholder="Judul Resep" value="{{ $resep->title }}" required>
        </div>
        <div class="mb-3">
            <label for="descriptions" class="form-label">Deskripsi Resep</label>
            <textarea id="descriptions" name="descriptions" class="form-control" placeholder="Deskripsi Resep" rows="3" required>{{ $resep->descriptions }}</textarea>
        </div>
        <div class="mb-3">
            <label for="ingredients" class="form-label">Bahan-Bahan</label>
            <textarea id="ingredients" name="ingredients" class="form-control" placeholder="Bahan-Bahan" rows="3" required>{{ $resep->ingredients }}</textarea>
        </div>
        <div class="mb-3">
            <label for="steps" class="form-label">Langkah-Langkah</label>
            <textarea id="steps" name="steps" class="form-control" placeholder="Langkah-Langkah" rows="4" required>{{ $resep->steps }}</textarea>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="cooking_time" class="form-label">Waktu Memasak (menit)</label>
                <input type="number" id="cooking_time" name="cooking_time" class="form-control" placeholder="Waktu Memasak" value="{{ $resep->cooking_time }}" required>
            </div>
            <div class="col-md-6 mb-3">
                <label for="portion" class="form-label">Porsi</label>
                <input type="number" id="portion" name="portion" class="form-control" placeholder="Porsi" value="{{ $resep->portion }}" required>
            </div>
        </div>
        <div class="mb-3">
        ...
        <button type="submit" class="btn btn-submit w-100 mt-3">Perbarui Resep</button>
    </form>
</div>
